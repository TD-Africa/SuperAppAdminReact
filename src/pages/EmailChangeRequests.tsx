...
export default function EmailChangeRequestsPage() {
  ...
  const columns: TableColumnsType<EmailChangeResponseWithUser> = [
    { title: "Company", dataIndex: "companyName", render: (v) => <span className="font-medium">{v ?? "—"}</span> },
    { title: "Old email", dataIndex: "oldEmail", render: (v) => <span className="text-xs">{v ?? "—"}</span> },
    { title: "New email", dataIndex: "newEmail", render: (v) => <span className="text-xs">{v}</span> },
    {
      title: "Submitted",
  ...
  return (
    <div className="space-y-6">
      ...
      <Card styles={{ body: { padding: 0 } }}>
        <Table<EmailChangeResponseWithUser>
          rowKey="id"
          dataSource={rows}
          columns={columns}
          ...
          locale={{ emptyText: "No email change requests." }}
        />
      </Card>
    </div>
  );
